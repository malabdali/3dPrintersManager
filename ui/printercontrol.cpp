...
{
    _device->GetDevicePort()->Write("G28 \n");
}

void PrinterControl::on__middle_button_clicked()
{
    _device->GetDevicePort()->Write("G1 F5000\n");
    _device->GetDevicePort()->Write("G1 F1200 Z30\n");
    _device->GetDevicePort()->Write((QString("G1 F5000 X%0 Y%1 \n").arg(_device->GetDeviceInfo()->GetX()/2).arg(_device->GetDeviceInfo()->GetY()/2)).toUtf8());
    _device->GetDevicePort()->Write("G1 F1200 Z0 \n");
}
...
{
    _device->GetDevicePort()->Write((QString("M104 S%0\n").arg(ui->_hotend->value())).toUtf8());
    _device->GetDevicePort()->Write((QString("M140 S%0\n").arg(ui->_bed->value())).toUtf8());
}

void PrinterControl::on__change_positiom_button_clicked()
{
    _device->GetDevicePort()->Write((QString("G1 F1000 Z%0 \n").arg(ui->_z->value())).toUtf8());
    _device->GetDevicePort()->Write((QString("G1 F3000 X%0 Y%1 \n").arg(ui->_x->value()).arg(ui->_y->value())).toUtf8());
